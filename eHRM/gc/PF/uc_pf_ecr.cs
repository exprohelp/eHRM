...
{
    public partial class uc_pf_ecr : UserControl
    {
        ...
        private void btn_Submit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string qry = "execute  pGC_Pf_queries 'ECR','"+rdp_date.Value.ToString("yyyy/MM/dd")+"','"+GlobalUsage.Login_id+"'";
        ...
        private void btnXL_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string fileName = mydocpath + "\\pf_ECR_"+rdp_date.Value.ToString("MMyyyy")+".xls";
        ...
        private void btnTextFile_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string fileName = mydocpath + "\\Pf_ECR_" + rdp_date.Value.ToString("MMyyyy") + ".txt";
                if (File.Exists(fileName))
                {
                    ...
                    using (TextWriter tw = new StreamWriter(fileName))
                    {
                        foreach (GridViewRowInfo gvr in rgv_info.Rows)
                        {
                            ...
                            {
                                if (gvc.ColumnInfo.Name.ToString() != "New")
                                {
                                    if (gvc.Value != null && gvc.Value.ToString().Length>0)
                                        if (gvc.ColumnInfo.Name.ToUpper().Contains("D O B") || gvc.ColumnInfo.Name.ToUpper().Contains("DOJ") || gvc.ColumnInfo.Name.ToUpper().Contains("DOE"))
